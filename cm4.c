  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	    ...
	    if (message_received)
	    {
	        ...
	        // Gelen mesajı UART8'e yazdır
	        HAL_UART_Transmit(&huart8, (uint8_t *)received_data_str, strlen(received_data_str), HAL_MAX_DELAY);

	        // CM7'ye cevap gönder ("Merhaba aldim")
	        const char *response = "Merhaba aldim";
	        OPENAMP_send(&rp_endpoint, (char *)response, strlen(response) + 1);
	    }
  }
  /* USER CODE END 3 */
}
